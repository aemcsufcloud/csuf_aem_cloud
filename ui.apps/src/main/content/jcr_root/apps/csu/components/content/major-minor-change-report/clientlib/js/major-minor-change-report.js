...
$(document).ready(function() {
    ...
    var wid = $(this).closest('tr').attr("data-id");
    var requestURL = '/bin/manageTask?action=TASK_URL&workItemId=' + encodeURIComponent(wid) + '&historyWorkItemId=' + '' + '&isInitialSubmission=' + false + '&isHistorySubmission=' + false;
                  ...
                  type : "GET",
                        ...
                        success : function(taskURL) { 
                            if(taskURL){                              
    							...
    							//window.open(window.location+taskURL, '_blank');
                                $("#frame").on("click", function() {
                                  $("task-iframe").insertBefore(".push");
                                });   

                                $('#task-iframe').attr('src', taskURL);
                                var url = document.getElementById('task-iframe').src;
   								var tabOrWindow = window.open(url, '_blank');
								//document.getElementById('task-iframe').contentWindow.document.getElementById('guideContainer-rootPanel-toolbar__').style.display = "none";
								//document.getElementById('task-iframe').contentWindow.document.getElementsByClassName('guidetoolbar')[0].style. Visibility = 'hidden';
  								tabOrWindow.focus();
    ...
    $('#task-iframe').load(function(){debugger;
 ...
 $.ajax({
        ...
        $.ajax({
            ...
            var selectedDelegateName = null;
                ...
                selectedDelegateEmail = "swathi.kumari@thoughtfocus.com";
 ...
 $.ajax({
    ...
    function delegateWorkItem(user, email, name, workflowInstanceId) {
        ...
        var bccEmail = "swathi.kumari@thoughtfocus.com";
