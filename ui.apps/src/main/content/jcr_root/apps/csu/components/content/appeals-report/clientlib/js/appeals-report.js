...
$(document).ready(function() {
    ...
    $(".fa-user-friends").on('click', function(event) {
        ...
        assignee = $(this).closest('tr').find('td:eq(11)').text();
 $.ajax({
        ...
        var dataUrl = "/bin/getDelegateUsersList?action=GET_DELEGATE_GROUP_MEMBERS&witemId=" + workflowInstanceId + "&assignee=" + encodeURIComponent(assignee)+ "&userId="+loggedInUserId;
        var data = {
            "url": (dataUrl)
        };
        $.ajax({
