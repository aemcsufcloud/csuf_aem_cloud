	...
	$.ajax({
		...
		success: function(myresponse) {
			...
			workflow_initiator.value = userValue;
          	hidden_employeeEmail.value = userValue +"@FULLERTON.EDU";
	...
	$.ajax({
		...
		success: function(myresponse) {
			...
			$.ajax({
				...
				data: {action: "STD_682_OVERTIME_USER_LOOKUP",
					userId: userID
				},
				dataType: 'json',
								...
								if (rButtons[n].checked === false) {
									rButtonStatus = false;
                                          ...
                                          chrsId.value = myresponse[n].CHRS_ID;
                                  		  hidden_chrsId.value = myresponse[n].CHRS_ID;
										  employee_last_name.value = myresponse[n].Last_Name;
										  employee_first_name.value = myresponse[n].First_Name;
										  empl_rcd.value = myresponse[n].EMPL_RCD;                             
										  employee_middle_name.value = myresponse[n].Middle_Name;
...
function getManager(empId,deptId,union_cd){
        ...
        $.ajax({
            type: 'GET',
            url: "/bin/getHourlyINTManagerUpdatedDetails",
            data: {
                empId: empId,
              	union_cd : union_cd,
                deptId: deptId,
                action: "MANAGER_DETAILS_EMPLID"
            },
            dataType: 'json',
            success: function(myresponse) {
                ...
                if (myresponse.length === 1) {                                  
                   manager_userID.value = myresponse[0].MANAGER_USERID;
        		   manager_email.value = myresponse[0].MANAGER_EMAIL_ID;   
        		// manager_email.value = "yjayaram@fullerton.edu";
        		   manager_name.value = myresponse[0].SupervisorName;				   
                } else{
                   manager_userID.value = "admin";
        		   manager_email.value = "nvadlakunta@fullerton.edu";   
        		   manager_name.value = "Administrator";	
                }
        ...
        $.ajax({
            ...
            success: function(myresponse) {
                if (myresponse.length === 1) {                  
                    time_keeper_name.value = myresponse[0].NAME;
                    time_keeper_email.value = myresponse[0].EMAILID;
                    //time_keeper_email.value = "yjayaram@fullerton.edu";
                    time_keeper_userID.value = myresponse[0].USERID;
                } 
...
/**
 * @function std_682_overtime_distributed_std_682_overtime_distributed.generated_chrsId_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
std_682_overtime_distributed_std_682_overtime_distributed.generated_chrsId_init0 = function (scope) {
    with(this) {
        with(scope) {
...
/**
 * @function std_682_overtime_distributed_std_682_overtime_distributed.generated_chrsId_init1
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
std_682_overtime_distributed_std_682_overtime_distributed.generated_chrsId_init1 = function (scope) {
    with(this) {
        with(scope) {
            if (StageIndicator.value !== null) {
  this.enabled = false;
...
if(StageIndicator.value === null){		 
	...
	var userValue = this.value; 
  	...
  	//hidden_employeeEmail.value = "yjayaram@fullerton.edu";
 
  	if(hidden_chrsId.value !==  this.value){

        $.ajax({

            type: 'GET', 
            url: '/bin/chrsIDUpdateServlet',   
            data: { action: "STD_682_OVERTIME_EMPL_LOOKUP",
                    chrsId: userValue,
                    userId: userId
            },
			dataType: 'json',
			...
			success: function(myresponse) {
				...
				if (myresponse.length === 1) {
                  ...
                  hidden_chrsId.value = this.value;
				  employee_last_name.value = myresponse[0].Last_Name;
				  employee_first_name.value = myresponse[0].First_Name;
				  empl_rcd.value = myresponse[0].EMPL_RCD;                             
				  employee_middle_name.value = myresponse[0].Middle_Name;
...
/**
 * @function std_682_overtime_distributed_std_682_overtime_distributed.generated_empl_Id_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
std_682_overtime_distributed_std_682_overtime_distributed.generated_empl_Id_init0 = function (scope) {
    with(this) {
        with(scope) {
            this.enabled = false;
        }
...
approving_official_signature.enabled = false;
        ...
        if (approving_official_date.value === null) {
          /*  var dateString = new Date().toLocaleString("en-US", {
            ...
            approving_official_date.value = getDateforAdaptiveForm();
            approving_official_signature.enabled = false;
        } else {
...
manager_signature.enabled = false;
        ...
        if (manager_date.value === null) {
          /*  var dateString = new Date().toLocaleString("en-US", {
            ...
            var curyearDay = dateObject.getDate();
            var d = (curyear + "-" + curyearMonth + "-" + curyearDay);*/
            manager_date.value = getDateforAdaptiveForm();
            manager_signature.enabled = false;
        } else {
...
time_keeper_signature.enabled = false;
        ...
        if (time_keeper_date.value === null) {
          /*  var dateString = new Date().toLocaleString("en-US", {
            ...
            var curyearDay = dateObject.getDate();
            var d = (curyear + "-" + curyearMonth + "-" + curyearDay);*/
            time_keeper_date.value = getDateforAdaptiveForm();
            time_keeper_signature.enabled = false;
        } else {
...
student_signature.enabled = false;
        if (student_date.value === null) {
          /*  var dateString = new Date().toLocaleString("en-US", {
            ...
            var curyearDay = dateObject.getDate();
            var d = (curyear + "-" + curyearMonth + "-" + curyearDay);*/
            student_date.value = getDateforAdaptiveForm();
            student_signature.enabled = false;
        } else {
...
aftiaDescCWID.value = employee_first_name.value +", " + employee_last_name.value + " " + chrsId.value;

EmailSubject.value = "Test - Request for STD 682 Overtime Distributed - " + employee_first_name.value + " " + employee_last_name.value + " ("+chrsId.value + ")";
