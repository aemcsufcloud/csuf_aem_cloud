                    ...
                    if (response[0].ACAD_CAREER == "UGRD") {
                        ...
                        if (response.length === 1) {
                           ...
                           // street.value = response[0].PREF_EMAIL;
                            ...
                            getChairData(response[0].DEPTID);
                            ...
                            okButton.onclick = function(event) {
                                ...
                                for (n = 0; n < rButtons.length; n++) {
                                    if (rButtons[n].checked === false) {
                                        ...
                                        getChairData(response[n].DEPTID);
...
if(StageIndicator.value === null){
  ...
  if(requestTypeFlag.value == "Winter" || requestTypeFlag.value == "Summer"){		
...
/**
 * @function request_for_excess_units_undergraduate_undergraduate_request_for_excess_units.generated_guideRootPanel_init2
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
request_for_excess_units_undergraduate_undergraduate_request_for_excess_units.generated_guideRootPanel_init2 = function (scope) {
    with(this) {
        with(scope) {
            if (StageIndicator.value === null) {
/*  $.ajax({
        ...
        success: function(myresponse) {
          ...
          if(userValue == "balyssa" || userValue == "quoiabrown" || userValue == "nashlaflores8104" || userValue == "snakehips" || userValue == "ygodinez" || userValue == "liamsummy" || userValue == "caitlin.pancia"){
            getStudentDetails();
          }else{
             showErrorModal("Alert!","The submission deadline has passed!");
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.ajax({
                ...
                type: 'GET',
                //url: '/bin/getExcessUnitDetails',
                url: '/bin/getExcessUnitsDataUpdated',
                data: {
                    userID: userValue,
                    action: 'STUDENT_DETAILS'
                },
                ...
                success: function(response) {
                    gifModal.style.display = "none";
                        ...
                        if (response.length === 1) {
                           ...
                           // street.value = response[0].PREF_EMAIL;
                            //hidden_student_email.value = response[0].PREF_EMAIL;
                              hidden_student_email.value = "yjayaram@fullerton.edu";
                            telephone.value = response[0].CELL_PHONE;
                            //debugger;
                            var streetAdd1 = "";
                            var streetAdd2 = "";
                            ...
                            okButton.onclick = function(event) {
                                ...
                                for (n = 0; n < rButtons.length; n++) {
                                    if (rButtons[n].checked === false) {
										...
										if (response[n].ACAD_CAREER == "UGRD") {
                                        CWID.value = response[n].EMPLID;
                                        studentName.value = response[n].FIRST_NAME + " " + response[n].LAST_NAME;
                                        //street.value = response[n].PREF_EMAIL;
                                       // hidden_student_email.value = response[n].PREF_EMAIL;
   ...
   $.ajax({
                ...
                success: function(response) {
                   hidden_chair_userID.value = response[0].CHAIR_USERID;
                            hidden_chair_name.value = response[0].CHAIR_NAME;
                            //hidden_chair_email.value = response[0].CHAIR_EMAIL;
                    hidden_chair_email.value = "yjayaram@fullerton.edu";
                }
   });
...
/**
 * @function request_for_excess_units_undergraduate_undergraduate_request_for_excess_units.generated_underGradUnits_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
request_for_excess_units_undergraduate_undergraduate_request_for_excess_units.generated_underGradUnits_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            if (this.value > 36 && StageIndicator.value === null) {
    false;
...
/**
 * @function request_for_excess_units_undergraduate_undergraduate_request_for_excess_units.generated_requestTypeFlag_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
request_for_excess_units_undergraduate_undergraduate_request_for_excess_units.generated_requestTypeFlag_init0 = function (scope) {
    with(this) {
        with(scope) {
            if (this.value == "Spring") {
   document.getElementById("springfall").innerHTML = "SPRING 2025" ;
   document.getElementById("springfall").style.fontWeight = "bold" ;
}else if(this.value == "Fall"){
  document.getElementById("springfall").innerHTML = "Fall 2025" ;
   document.getElementById("springfall").style.fontWeight = "bold" ;
}else{
   document.getElementById("springfall").innerHTML = "Winter 2026" ;
   document.getElementById("springfall").style.fontWeight = "bold" ;
}
...
if (StageIndicator.value == "ToAdvisor" || StageIndicator.value == "ToARSC" || StageIndicator.value == "ToChair") {
   // if(this.value == "Spring 2022"){
     var term = this.value;
     if((term.includes("Spring")) || (term.includes("Fall"))){
      summerHeaderText.visible = false;
...
function submitAction(){
	...
	EmailSubject.value = "Test - Request of Excess Unit (Undergraduate) - " + CWID.value;
...
function springFallValidation(){
	...
	else if(underGradCumulative_GPA.value === null){
	...
	else if(underGradReason.value === null){
