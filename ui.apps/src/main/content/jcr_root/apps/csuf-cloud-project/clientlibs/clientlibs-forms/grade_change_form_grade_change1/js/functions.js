...
$.ajax({
    ...
    success: function(myresopnse) {
        ...
        if (userValue === "mcalarco") {
            var popupResult = confirm(
                "Click OK to launch for Self.\nClick Cancel to launch for John Davis."
            );
            if (popupResult) {
                startExistingFlow(userValue);
            } else {
                startExistingFlow("johndavis");
            }
        } else {
            startExistingFlow(userValue);
        }

        function startExistingFlow(userValue) {
            if (StageIndicator.value === null) {
                                        ...
                                        success: function(myresponse) {
                                            ...
                                            if (myresponse.length >= 1) {
                                                ...
                                                /*	  var courseName_NumberSelect = document.querySelector(".CourseName_NumberSelect select");
                                                              ...
                                                              */
                                                Class_CourseName.items = "";
                                                Class_CourseName.value = "";
                                                var courseArray = [];
                                                for (var i = 0; i < myresponse.length; i++) {
                                                    courseArray.push(myresponse[i].clsCrs);
                                                }
                                                Class_CourseName.items = courseArray;
...
$.ajax ({
  		...
  		success: function(myresponse){
                      ...
                      var sectionarray = [];
                      for(var n=0; n < myresponse.length; n++){
                        sectionarray.push(myresponse[n].classSection);
                            }
                      
                      SectionNumber.items = sectionarray;
                    } 
          			gifModal.style.display = "none";
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    ...
                    var courseID = response[0].CRSE_ID;
                            ...
                            if (myresponse.length > 0) {
                                ...
                                for (m = 0; m < myresponse.length; m++) {
                                    if (myresponse.length == 1) {
                                        ...
                                        GradingBasisHidden.value = myresponse[1].GRADING_BASIS;
                                        GradingSchemeHidden.value = myresponse[1].GRADING_SCHEME;
                                    }
                                       ...
                                       /* var length = clearGrades.options.length;
                                        ...
                                        if (gradeArr === undefined) {
                                            ...
                                            for (var j = 0; j < myresponse.length; j++) {
                                                toList.push(myresponse[j].CRSE_GRADE_INPUT);                                             
											} // for add option elements

                                            //  }// for get select element 
                                        } else if (gradeArr.length > 0) {


                                            // for (var i=0; i< gradeChangeToSelectList.length; i++){

                                            for (var j = 0; j < gradeArr.length; j++) {
                                                toList.push(gradeArr[j]);                                               

                                            } // for add option elements
                                          
                                          if(courseID == '018134' && GradingBasis == 'GRD' && GradingScheme == 'UGR'){
                                            toList.push('CR'); 
                                          }

                                        } // for get select element 

//loop and check for grades
var k;
                                    var rowCount = Row1.instanceManager.instanceCount;
for(k=0;k< rowCount;k++){	  
                        Row1.instanceManager.instances[k].GradeChangeTo.items = toList;
                        var toOptions = Row1.instanceManager.instances[k].GradeChangeTo.items;
  debugger;
                        if (Row1.instanceManager.instances[k].GradeChangeFrom.value == "CR") {
                            toOptions.push("NC");
                            // added below 3 lines on 04232024
                            jsonObject = toOptions.map(JSON.stringify);
                            uniqueSet = new Set(jsonObject);
                            uniqueArray = Array.from(uniqueSet).map(JSON.parse);
                            Row1.instanceManager.instances[k].GradeChangeTo.items = uniqueArray;
                        } else if (Row1.instanceManager.instances[k].GradeChangeFrom.value == "NC") {
                            toOptions.push("CR");
                            // added below 3 lines on 04232024
                            jsonObject = toOptions.map(JSON.stringify);
                            uniqueSet = new Set(jsonObject);
                    ...
                    for (i = 0; i < response.length; i++) {
                        ...
                        Row1.instanceManager.instances[i].StudentLastName.value = response[i].LNAME;
                        Row1.instanceManager.instances[i].StudentLastName.enabled = false;
                        Row1.instanceManager.instances[i].StudentFirstName.value = response[i].FNAME;
                        Row1.instanceManager.instances[i].StudentFirstName.enabled = false;
                        Row1.instanceManager.instances[i].SID.value = response[i].CWID;
                        Row1.instanceManager.instances[i].SID.enabled = false;
                        Row1.instanceManager.instances[i].UnitTaken.value = response[i].UNT_TAKEN;
                        Row1.instanceManager.instances[i].UnitTaken.enabled = false;
                        Row1.instanceManager.instances[i].GradeChangeFrom.value = response[i].CURRENT_GRADE;
                    ...
                    // Getting all the information from database on form
                    InstructorName.value = response[0].INSTR_NAME;
                    DeptCode.value = response[0].DEPT_CD;
                    CourseLevel.value = response[0].COURSE_LEVEL;
                    HiddenCourseID.value = response[0].CRSE_ID;
                    var instName = response[0].INSTR_NAME;
                    ...
                    HiddenSchduleNBR.value = response[0].SCHEDULE_NBR;
                    //HiddenCourseName.value = myresponse[0].CRSE_NAME ;
                    HiddenStudentEnrolStatus.value = response[0].STDNT_ENRL_STATUS;
                    HiddenMajorType.value = response[0].MAJOR_TYPE;
                    HiddentTermID.value = response[0].STRM;
                    HiddenTermDescription.value = response[0].TERM_DESCR;
                    HiddenDepartmentCode.value = response[0].DEPT_CD;
                    HiddenMajorCode.value = response[0].MAJOR_CODE;
                    HiddenCollege.value = response[0].COLLEGE;
                    HiddenMajorDescription.value = response[0].MAJOR_DESCR;
                    HiddenDegreeType.value = response[0].DEGREE_TYPE;
                    HiddenEffSEQ.value = response[0].EFFSEQ;
                    HiddenInstituation.value = response[0].INSTITUTION;
                    HiddenEffDT.value = response[0].EFFDT;
                    HiddenAcademicProg.value = response[0].ACAD_PROG;
                    HiddenUnitTaken.value = response[0].UNT_TAKEN;
                    HiddenStudentMiddleName.value = response[0].MNAME;
                    HiddenStudentCarNBR.value = response[0].STDNT_CAR_NBR;
                    HiddenStudentUserID.value = response[0].STUDENT_USERID;
                    HiddenStudentID.value = response[0].STUDENT_EMAIL;
                    HiddenInstructorCWID.value = response[0].INSTR_CWID;
                    HiddenInstructorUserID.value = response[0].INSTR_USERID;
                    HiddenInstructorEmail.value = response[0].INSTR_EMAIL;
                    HiddenChairCWID.value = response[0].CHAIR_CWID;
                    HiddenChairName.value = response[0].CHAIR_NAME;
                    HiddenChairUserID.value = response[0].CHAIR_USERID;
                    HiddenChairEmail.value = response[0].CHAIR_EMAIL;
                    var deanDetails = getAlternativeDean(response[0].DEAN_USERID);
                   
                    if (deanDetails !== null) {
                        HiddenDeanCWID.value = deanDetails[0].DEAN_CWID;
                        HiddenDeanName.value = deanDetails[0].DEAN_NAME;
                        HiddenDeanUserID.value = deanDetails[0].DEAN_USERID;
                        HiddenDeanEmail.value = deanDetails[0].DEAN_EMAIL;
                    } else {
                        HiddenDeanCWID.value = response[0].DEAN_CWID;
                        HiddenDeanName.value = response[0].DEAN_NAME;
                        HiddenDeanUserID.value = response[0].DEAN_USERID;
                        HiddenDeanEmail.value = response[0].DEAN_EMAIL;
                        HiddenAssociateDeanCWID.value = response[0].ASS_DEAN_CWID;
                        HiddenAssociateDeanName.value = response[0].ASS_DEAN_NAME;
                        HiddenAssociateDeanUserID.value = response[0].ASS_DEAN_USERID;
                        HiddenAssociateDeanEmail.value = response[0].ASS_DEAN_EMAIL;
                    }
                  
                   if(HiddenChairUserID.value === null){
                    HiddenChairUserID.value = "admin";
                  }
                  if(HiddenChairEmail.value === null){
                    HiddenChairEmail.value = "csuf@fullerton.edu";
                  }
                  if(HiddenDeanUserID.value === null){
                    HiddenDeanUserID.value = "admin";
                  }
                  if(HiddenDeanEmail.value === null){
                    HiddenDeanEmail.value = "csuf@fullerton.edu";
                  }
    ...
    with(this) {
        with(scope) {
           ...
           /* var dateString = new Date().toLocaleString("en-US", {
            ...
            InstructorDate.value = getDateforAdaptiveForm();
    ...
    with(this) {
        with(scope) {
           ...
           /* var dateString = new Date().toLocaleString("en-US", {
            ...
            var curyearDay = dateObject.getDate();
            var d = (curyear + "-" + curyearMonth + "-" + curyearDay);*/
            ChairDate.value = getDateforAdaptiveForm();
    ...
    with(this) {
        with(scope) {
          ...
          /*  var dateString = new Date().toLocaleString("en-US", {
            ...
            var curyearDay = dateObject.getDate();
            var d = (curyear + "-" + curyearMonth + "-" + curyearDay);*/
            DeanDate.value = getDateforAdaptiveForm();
    ...
    with(this) {
        with(scope) {
          ...
          /*  var dateString = new Date().toLocaleString("en-US", {
            ...
            var curyearDay = dateObject.getDate();
            var d = (curyear + "-" + curyearMonth + "-" + curyearDay);*/
            CMSUpdateDate.value = getDateforAdaptiveForm();
