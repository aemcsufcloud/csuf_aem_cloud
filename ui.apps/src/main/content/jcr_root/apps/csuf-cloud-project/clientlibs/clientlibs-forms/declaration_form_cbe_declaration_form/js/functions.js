  	 ...
  	 $.ajax({
        ...
        success: function(response) {
          	...
          	if(response.length > 0){              	
               ...
               // HiddenAdvisorEmail.value = response[0].EMAILID;
               HiddenAdvisorEmail.value ="shreyas.manjunatha@thoughtfocus.com";
            } 
          	else{
...
/**
 * @function declaration_form_cbe_declaration_form.generated_cwid_valueCommit1
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
declaration_form_cbe_declaration_form.generated_cwid_valueCommit1 = function (scope) {
    with(this) {
        with(scope) {
            if (AdvisorSignaturePanel.visible === true && ARSCPanel.visible === false && RecordsOfficeUseOnlyPanel.visible === false) {
    var gifModal = document.getElementById('gifModal');
        ...
        $.ajax({
            ...
            success: function(myresponse) {
                if (myresponse.length >= 1) {
                    ...
                    if (myresponse.length > 1) {
                        for (var i = 0; i < myresponse.length; i++) {
                            if (myresponse[i].ACAD_PLAN_TYPE == "MAJ") {
                                major = myresponse[i].PROGRAMS;
                                acadPlan = myresponse[i].ACAD_PLAN;
                                admitTerm = myresponse[i].ADMIT_TERM_DESCR;
                                resArray[count] = {};
                                resArray[count].PROGRAMS = major;
                                resArray[count].ACAD_PLAN =  acadPlan;
                                resArray[count].ADMIT_TERM_DESCR =  admitTerm;
                                count = count + 1;
                            }
                        }
                    }
                    if (count == 1) {
                        ...
                        CurrentMajor.value = major;
                        CurrentMajorAcadPlan.value = acadPlan;
                        ...
                        var headings = ["", "CWID", "First Name", "Last Name", "Admit Term", "Program", "Acad Plan"];
                        ...
                        for (var k = 0; k < resArray.length; k++) {
                            ...
                            tabCell4.innerHTML = cwidVal;
                            var tabCell2 = tr.insertCell(-1);
                            tabCell2.innerHTML = myresponse[0].STUDENT_FNAME;
                            var tabCell3 = tr.insertCell(-1); 
                            tabCell3.innerHTML = myresponse[0].STUDENT_LNAME;
                            for (var l = 0; l < col.length; l++) {
                                var tabCell = tr.insertCell(-1);
                                tabCell.innerHTML = resArray[k][col[l]];
                            }
                        }
                        ...
                        okButton.onclick = function(event) {
                            ...
                            for (n = 0; n < rButtons.length; n++) {
                                if (rButtons[n].checked === false) {
                                    ...
                                    AdmitTerm.value = resArray[n].ADMIT_TERM_DESCR;
                                    currentMajor.value = resArray[n].PROGRAMS;
                                    CurrentMajor.value = resArray[n].PROGRAMS;
                                    CurrentMajorAcadPlan.value = resArray[n].ACAD_PLAN;
                                    rButtonStatus = true;
                                    break;
                                }
                            }
                    ...
                    FName.value = myresponse[0].STUDENT_FNAME;
                    LName.value = myresponse[0].STUDENT_LNAME;
                 //   AdmitTerm.value = myresponse[0].ADMIT_TERM_DESCR; // Commented on 07142023
                 //   currentMajor.value = myresponse[0].PROGRAMS; // Commented on 07142023
                 //   CurrentMajor.value = myresponse[0].PROGRAMS; // Commented on 07142023
                 //   CurrentMajorAcadPlan.value = myresponse[0].ACAD_PLAN; // Commented on 07142023
                 //   HiddentStudentEmailID.value = myresponse[0].STUDENT_EMAIL;
                   HiddentStudentEmailID.value ="shreyas.manjunatha@thoughtfocus.com";
                    getCourseDetails(myresponse[0].STUDENT_USERID);
                    /*if (myresponse[0].ACAD_PLAN == "42PBUSUND") {
                        formerMajorRB.value = 1;
                    } 
            ...
            if (myresponse.length >= 1) {
              ...
              var array = [];
                for(m=0; m<myresponse.length; m++){
                  if(myresponse[m].TERM_DESCR == "Spring 2024"){
                    array.push(myresponse[m]);
                  }
                }debugger;
              if(array.length >= 1){
                gifModal.style.display = "block";
                LookupResult.value = JSON.stringify(array);
                var k;
                var rowcountRemoveAll1 = CourseRow.instanceManager.instanceCount;
                for (k = 0; k < rowcountRemoveAll1; k++) {
                    CourseRow.instanceManager.removeInstance(CourseRow.instanceIndex);
                ...
                for (i = 0; i < array.length; i++) {
                    CourseRow.instanceManager.addInstance();
                    CourseRow.instanceManager.instances[i].CourseNo.value = array[i].CRSE_NAME;
                    CourseRow.instanceManager.instances[i].ScheduleNo.value = array[i].CLASS_NBR;
                    CourseRow.instanceManager.instances[i].NumberOfUnits.value = array[i].UNT_TAKEN;
                    CourseRow.instanceManager.instances[i].NameOfInstructor.value = array[i].INSTR_NAME;
                    CourseRow.instanceManager.instances[i].SectionNo.value = array[i].CLASS_SECTION;
                }
                var rowcount = CourseRow.instanceManager.instanceCount;
                CourseRow.instanceManager.removeInstance(rowcount - 1);
                gifModal.style.display = "none";
                ...
                var p;
                var rowcountRemoveAll3 = CourseRow.instanceManager.instanceCount;
                for (p = 0; p < rowcountRemoveAll3; p++) {                    
                    CourseRow.instanceManager.removeInstance(CourseRow.instanceIndex);
                }
                var rowcount3 = CourseRow.instanceManager.instanceCount;
                CourseRow.instanceManager.instances[rowcount3 - 1].CourseNo.value = null;
                CourseRow.instanceManager.instances[rowcount3 - 1].ScheduleNo.value = null;
                CourseRow.instanceManager.instances[rowcount3 - 1].NumberOfUnits.value = null;
                CourseRow.instanceManager.instances[rowcount3 - 1].NameOfInstructor.value = null;
                CourseRow.instanceManager.instances[rowcount3 - 1].SectionNo.value = null;
                CourseRow.instanceManager.removeInstance(rowcount3 - 1);
...
/**
 * @function declaration_form_cbe_declaration_form.generated_ARSCCB_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
declaration_form_cbe_declaration_form.generated_ARSCCB_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            if(StageIndicator.value === "ToARSC"){
if(this.value == 1){
...
/**
 * @function declaration_form_cbe_declaration_form.generated_HiddenPanel_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
declaration_form_cbe_declaration_form.generated_HiddenPanel_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            this.visible=false;
