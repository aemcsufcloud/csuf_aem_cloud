...
if (myresponse.length !== 0) {
                    ...
                    for (var i = 0; i <  myresponse.length; i++) {
                      debugger;
                        ...
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].REFCaseID.value = myresponse[i].DETAILS[j].CASE_ID;
                          if( Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].ReturnStatus.value == "1"){
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].Hook.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].Copy.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].Building.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].Room.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].ResponsibleSignature.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].IssuedDate.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].ReturnDate.enabled =  false;
						Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].ReturnStatus.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].REFWorkflowInstanceID.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceManager.instanceCount-1].REFCaseID.enabled =  false;
                          }
						}
                    }
 ...
 showErrorModal("Alert!", "Please fill CampusID");
  ...
  ApplicantSignatureDate.value = ""; 
  AuthorizingSignature.value = ""; 
  AuthorizingSignatureDate.value = ""; 
  WorkflowInstanceID.value = "";
  for (var i=0; i<KeyTable.Row1.instanceManager.instanceCount; i++){
                         KeyTable.Row1.instanceManager.instances[i].Hook.value =  "";
                         KeyTable.Row1.instanceManager.instances[i].Copy.value =  "";
                         KeyTable.Row1.instanceManager.instances[i].Building.value = "";
                         KeyTable.Row1.instanceManager.instances[i].Room.value =  "";
                         KeyTable.Row1.instanceManager.instances[i].ResponsibleSignature.value =  "";
                         KeyTable.Row1.instanceManager.instances[i].IssuedDate.value =  "";
                         KeyTable.Row1.instanceManager.instances[i].ReturnDate.value =  "";
						 KeyTable.Row1.instanceManager.instances[i].ReturnStatus.value =  "";
                         KeyTable.Row1.instanceManager.instances[i].REFWorkflowInstanceID.value = "";
                         KeyTable.Row1.instanceManager.instances[i].REFCaseID.value = "";
     }
...
/**
 * @function return_blue_card_university_key_control___building_keys___return.generated_ReturnDate_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
return_blue_card_university_key_control___building_keys___return.generated_ReturnDate_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            if(this.value<IssuedDate.value){

  showErrorModal("Alert!", "Return date can't be before issued date");
}
        }
	}
}
/**
 * @function return_blue_card_university_key_control___building_keys___return.generated_ReturnStatus_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
return_blue_card_university_key_control___building_keys___return.generated_ReturnStatus_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            if(StageIndicator.value === null){
  if(this.value==1){
     for (var i=0; i<KeyTable.Row1.instanceManager.instanceCount; i++){
    if(KeyTable.Row1.instanceManager.instances[i].ReturnStatus.value !== null && KeyTable.Row1.instanceManager.instances[i].ReturnDate.value === null){
      KeyTable.Row1.instanceManager.instances[i].ReturnDate.mandatory= true;
    }
     }
  }
}
...
/**
 * @function return_blue_card_university_key_control___building_keys___return.generated_headerItem16503489436141650348944228_click0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
return_blue_card_university_key_control___building_keys___return.generated_headerItem16503489436141650348944228_click0 = function (scope) {
    with(this) {
        with(scope) {
                                   

                        Row1.instanceManager.instances[Row1.instanceIndex].Hook.enabled =  true;
                        Row1.instanceManager.instances[Row1.instanceIndex].Copy.enabled =  true;
                        Row1.instanceManager.instances[Row1.instanceIndex].Building.enabled =  true;
                        Row1.instanceManager.instances[Row1.instanceIndex].Room.enabled =  true;
                        Row1.instanceManager.instances[Row1.instanceIndex].ResponsibleSignature.enabled =  true;
                        Row1.instanceManager.instances[Row1.instanceIndex].ReturnDate.enabled =  true;
						Row1.instanceManager.instances[Row1.instanceIndex].ReturnStatus.enabled =  true;
                        Row1.instanceManager.instances[Row1.instanceIndex].REFWorkflowInstanceID.enabled =  false;
                        Row1.instanceManager.instances[Row1.instanceIndex].REFCaseID.enabled =  false;
        }
	}
}
/**
...
return_blue_card_university_key_control___building_keys___return.generated_submit1574920582933_click0 = function (scope) {
  ...
  EmailSubject.value = "Test - University Key Control (Return)- Building Keys "+Name.value;
