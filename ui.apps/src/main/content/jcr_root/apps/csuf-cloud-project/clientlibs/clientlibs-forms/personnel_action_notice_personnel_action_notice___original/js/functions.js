...
/**
 * @function personnel_action_notice_personnel_action_notice___original.generated_ChrsID_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
personnel_action_notice_personnel_action_notice___original.generated_ChrsID_init0 = function (scope) {
    with(this) {
        with(scope) {
...
/**
 * @function personnel_action_notice_personnel_action_notice___original.generated_ChrsID_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
personnel_action_notice_personnel_action_notice___original.generated_ChrsID_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
				...
				success: function(myresponse){
					  ...
					  $.ajax({
							...
							data: {
								//cwid: cwid
								//userID : userID
								action : "PERSONNEL_ACTION_NOTICE_CHRSID",
                                chrsId : chrsId
							},
							dataType: 'json',
...
/**
 * @function personnel_action_notice_personnel_action_notice___original.generated_EmplID_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
personnel_action_notice_personnel_action_notice___original.generated_EmplID_init0 = function (scope) {
    with(this) {
        with(scope) {
            this.enabled = false;
        }
...
personnel_action_notice_personnel_action_notice___original.generated_HRCB_valueCommit0 = function (scope) {
    ...
    if(this.value == 1){
            ...
            if (HRDate.value === null) {
                ...
                HRDate.value = getDateforAdaptiveForm();
                HRName.enabled = false;
            } else {
    ...
    //if (StageIndicator.value == "ToHRCoo") {
        if (PayrollName.value === null) {
           /* var dateString = new Date().toLocaleString("en-US", {
            ...
            var curyearDay = dateObject.getDate();
            var d = (curyear + "-" + curyearMonth + "-" + curyearDay);*/
            PayrollDate.value = getDateforAdaptiveForm();
...
EmailSubject.value = "Personnel Action Notice Request - " + EmplID.value;*/

aftiaDescCWID.value = (FirstName.value + " " + LastName.value + " " + ChrsID.value);

EmailSubject.value = "Test - Personnel Action Notice Request - " + ChrsID.value;
