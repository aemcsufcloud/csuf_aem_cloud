...
/**
 * @function f1gone_request_to_cancel_financial_aid_processing_request_to_cancel_financial_aid_processing.generated_guideRootPanel_init2
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
f1gone_request_to_cancel_financial_aid_processing_request_to_cancel_financial_aid_processing.generated_guideRootPanel_init2 = function (scope) {
    with(this) {
        with(scope) {
            debugger;
if (StageIndicator.value === null) {
...
function loggedInDetails() {
    $.ajax({
        ...
        success: function(response) {
            // gifModal.style.display = "block";
            //var userValue = response.userId;
             var userValue = 'akuntsevich';	 	  	// two Aid Year
            //var userValue = 'majesticallexi'; // one Aid Year
            //var userValue = 'mchoi88';			 	// no Aid Year
            workflow_initiator.value = userValue;
            caseID();
...
function singleAidYear() {
    ...
    if (typeOfAidYear == '0') {
        ...
        financialAidYear.value = financialAidYearVal;
        aidYearValue = financialAidYear.value;
        formCode.value = financialAidvalues.FormCodeGeneral+"GONE";
        formCodeVal = financialAidvalues.FormCodeGeneral+"GONE";
        getFAFSAFinancialAidYear(aidYearValue);
        checkforDuplicateSubmissions(formCodeVal, financialAidYearVal); 
    }
}
function aidYearPopup() {
    ...
    document.getElementById("button1").onclick = function() {
        ...
        financialAidYear.value = financialAidYearVal;
        aidYearValue = financialAidYear.value;
        formCode.value = financialAidvalues.FinAidYearFormCodeOne+"GONE";
        formCodeVal = financialAidvalues.FinAidYearFormCodeOne+"GONE";
        getFAFSAFinancialAidYear(aidYearValue);
        checkforDuplicateSubmissions(formCodeVal, financialAidYearVal);
    };
    document.getElementById("button2").onclick = function() {
        ...
        financialAidYear.value = financialAidYearVal;
        aidYearValue = financialAidYear.value;
        formCode.value = financialAidvalues.FinAidYearFormCodeTwo+"GONE";
        formCodeVal = financialAidvalues.FinAidYearFormCodeTwo+"GONE";
        getFAFSAFinancialAidYear(aidYearValue);
        checkforDuplicateSubmissions(formCodeVal, financialAidYearVal);
    };
}
...
function getFAFSAFinancialAidYear(aidYearValue) { 
    ...
    var aidYearVal1=aidYear.value;
    var aidYearVal2=aidYearVal1-1;
  
    var formCodeTextVal = "<p><b>"+formCode.value+"</b></p>";
    var titleTextVal = "<p><b>REQUEST TO CANCEL FINANCIAL AID PROCESSING (".concat(financialAidYearVal).concat(")</b></p>");
    var instructionsText = "<p>Use this form to request <b>full cancellation</b> of your financial aid application for the ".concat(financialAidYearVal).concat(" academic year. To revise your aid award to <b>one semester</b> complete the Award Adjustment Appeal Form.</p>");
    var cb1 = "<p>I will not attend Cal State Fullerton during the ".concat(financialAidYearVal).concat(" academic year. Please cancel my financial aid application and, if funds have been awarded to me, please cancel all awards.</p>");
    var cb2 = "<p>I plan to attend CSUF but wish to cancel the processing of my financial aid for ".concat(financialAidYearVal).concat(".</p>");
    var cb3 = "<p>I attended classes in Fall ".concat(aidYearVal2).concat("</p>");
    var cb4 = "<p>I attended classes in Spring ".concat(aidYearVal1).concat("</p>");
...
function submitAction() {
    ...
    EmailSubject.value = "Test - Request to cancel Financial Aid Processing - (" + cwid.value + ")";
