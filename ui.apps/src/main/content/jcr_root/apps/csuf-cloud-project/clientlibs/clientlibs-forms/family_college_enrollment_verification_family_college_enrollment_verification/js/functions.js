...
function getCDAFinancialAidYear(financialAidYear) {
    ...
    var headingTextVal = "<p><b>FAMILY COLLEGE ENROLLMENT VERIFICATION (".concat(financialAidYear).concat(")</b></p>");

    var formTextVal = "<p><b>"+formCode.value+"</b></p>";
    var sectionTextVal = "<p>in ".concat(financialAidYear).concat("</p>");
    var instructionTextVal = "<p><b><u>INSTRUCTIONS:</u></b>We need to confirm that a member of your family will be enrolled in college at least half-time during the ".concat(financialAidYear).concat(" academic year. The family member must be working towards a degree or certificate leading to a recognized educational credential at a college that is eligible to participate in federal student aid programs. Please have that person sign and submit this form to his/her financial aid office for completion of the review. If you have more than one family member in college, make a photocopy of this form for each person. If your family member will not be enrolled in college at least half-time during the ".concat(financialAidYear).concat(" academic year, please check the appropriate box, sign the certification and submit it to the Office of Financial Aid at California State University. <i>They will be disposed of in a secure manner, per university policy. This will delay processing.</i></p>"));
...
/**
 * @function family_college_enrollment_verification_family_college_enrollment_verification.generated_SSN_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
family_college_enrollment_verification_family_college_enrollment_verification.generated_SSN_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            var ssndig = SSN.toString().length;
if(ssndig.value < 4){
...
if (StageIndicator.value === null) {
    ...
    EmailSubject.value = "Test - Family College Enrollment Verification Form - " + cwid.value;
}
//StudentEmailId.value = "yjayaram@fullerton.edu";
StudentEmailId.value = "anupama.dhar@thoughtfocus.com";
...
if(CertificationRB.value === null){
  ...
  showErrorModal("Alert!", "Please enter valid SSN");
...
else if((CertificationRB.value == 1) && (ParentEmail.value === null)){
    showErrorModal("Alert!", "Please enter Family Member Email address");
    ...
    showErrorModal("Alert!", "Family Member Email does not match");
  	guideBridge.setFocus("guide[0].guide1[0].guideRootPanel[0].MainPanel[0].SectionPanel[0].ConfirmParentEmail[0]");
}

else if((CertificationRB.value == 1) && (AdminEmail.value === null)) {
    showErrorModal("Alert!", "Please enter Administrator email address");
    ...
    showErrorModal("Alert!", "Administrator email does not match");
  ...
  documentNameForAdobeSign.value = "Family College Enrollment Verification Form " + formCode.value;
  guideBridge.submit();
}
        }
	}
