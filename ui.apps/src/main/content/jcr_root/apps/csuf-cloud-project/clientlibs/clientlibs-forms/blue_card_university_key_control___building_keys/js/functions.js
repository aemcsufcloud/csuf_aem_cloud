...
if(StageIndicator.value==="ToPolice"){
  ...
  for (var i=0; i<KeyTable.Row1.instanceManager.instanceCount; i++){
    if(KeyTable.Row1.instanceManager.instances[i].Room.value !== null && KeyTable.Row1.instanceManager.instances[i].Building.value !== null){
      KeyTable.Row1.instanceManager.instances[i].IssuedDate.mandatory= true;
    }
  }
   }
...
if (myresopnse.length === 1 && myresopnse[0].EMPLID!== undefined) {
   debugger;
       ...
       /* var employmenttype = myresopnse[0].FULL_PART_TIME;
        if(employmenttype == "P"){
...
/**
 * @function blue_card_university_key_control___building_keys.generated_CampusID_valueCommit1
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
blue_card_university_key_control___building_keys.generated_CampusID_valueCommit1 = function (scope) {
    with(this) {
        with(scope) {
            debugger;
if(StageIndicator.value === null){
	...
	success: function(myresponse){
      debugger;
		if(myresponse.length !== 0){
			CampusPhone.value = myresponse[0].PHONE;
          var position = myresponse[0].POSITION;
          if(position == "Staff"){
            EmploymentType.value=4;
          }
	}
	}
});
...
/**
 * @function blue_card_university_key_control___building_keys.generated_Building_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
blue_card_university_key_control___building_keys.generated_Building_init0 = function (scope) {
    with(this) {
        with(scope) {
            debugger;
if(StageIndicator.value === null){
$.ajax({
	type: 'GET',
	url: "/bin/getUniversityKeyControlBuildingInfo",
	data: {
		action:"UKC_BUILDING_DATA",
	},
	dataType: 'json',
	success: function(myresponse){
      debugger;
		 if (myresponse.length !== 0) {
			 ...
			 var item = myresponse[i].BUILDING_NAME;
			 var idItem = i+1;
			 buildingList.push(item);
			 }
           for(var j=0; j<KeyTable.Row1.instanceManager.instanceCount; j++){
		KeyTable.Row1.instanceManager.instances[j].Building.items = (buildingList);
           }
...
/**
 * @function blue_card_university_key_control___building_keys.generated_Building_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
blue_card_university_key_control___building_keys.generated_Building_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            if(StageIndicator.value === null){
  debugger;
  var buildingName = Row1.instanceManager.instances[Row1.instanceIndex].Building.value; 
  Row1.instanceManager.instances[Row1.instanceIndex].Room.value = "";
  var code="";
var buildingParsedArray = JSON.parse(BuildingListJson.value);
  for(a=0; a<buildingParsedArray.length; a++ ){
    debugger;
    if(buildingParsedArray[a].BUILDING_NAME == buildingName){
      code=buildingParsedArray[a].CODE;
    }
  }
$.ajax({
	type: 'GET',
	...
	success: function(myresponse){
      debugger;
		 if (myresponse.length !== 0) {
			 var areaList = [];
			 for (var i=0; i<myresponse.length; i++){
			 var item = myresponse[i].AREA_DESCRIPTION;
			 var idItem = i+1;
			 areaList.push(item);
			 }
			 Row1.instanceManager.instances[Row1.instanceIndex].Room.items = (areaList);
		 }
	},
	error: function(error) {
alert("error block=" + error);
...
/**
 * @function blue_card_university_key_control___building_keys.generated_EmployeeFinalCB_valueCommit0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
blue_card_university_key_control___building_keys.generated_EmployeeFinalCB_valueCommit0 = function (scope) {
    with(this) {
        with(scope) {
            debugger;
if(StageIndicator.value == "ToPolice"){
  if(this.value === "1"){
   ...
   for (var i=0; i<KeyTable.Row1.instanceManager.instanceCount; i++){
    if(KeyTable.Row1.instanceManager.instances[i].Room.value !== null && KeyTable.Row1.instanceManager.instances[i].Building.value !== null){
      KeyTable.Row1.instanceManager.instances[i].ResponsibleSignature.value = Name.value;
