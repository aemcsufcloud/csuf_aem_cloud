...
/**
 * @function parent_federal_tax_return_schedule_e_parent_federal_tax_return_schedule_e.generated_guideRootPanel_init2
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
parent_federal_tax_return_schedule_e_parent_federal_tax_return_schedule_e.generated_guideRootPanel_init2 = function (scope) {
    with(this) {
        with(scope) {
            var flag;
if (StageIndicator.value === null) {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.length >= 1) {
                ...
                cwid.value = studentCWID;
                //StudentEmailId.value = response[0].student_Email;
                HiddenStudentEmail.value = "shreyas.manjuntha@thoughtfocus.com";
                HiddenStudentName.value = response[0].FIRST_NAME + " " + response[0].LAST_NAME;
                firstName.enabled = false;
                lastName.enabled = false;
                cwid.enabled = false;
...
function singleAidYear() {
    ...
    if (typeOfAidYear == '0') {
        ...
        financialAidYear.value = financialAidYearVal;
        aidYearValue = financialAidYear.value;
        formCode.value = financialAidvalues.FormCodeGeneral + "SLEP";
        formCodeVal = financialAidvalues.FormCodeGeneral + "SLEP";
        getCDAFinancialAidYear(aidYearValue);
        checkforDuplicateSubmissions(formCodeVal, financialAidYearVal);
    }
...
function getCDAFinancialAidYear(financialAidYear) {
    ...
    var headingTextVal = "<p><b>PARENT ".concat(taxFilingYear).concat(" FEDERAL TAX RETURN SCHEDULE E (".concat(financialAidYear).concat(")</b></p>"));
    var formTextVal = "<p><b>" + formCode.value + "</b></p>";
    var TextOne = "<p><b><u>INSTRUCTIONS:</u></b>&nbsp; Additional information is needed to determine your financial aid eligibility for the ".concat(financialAidYear).concat(" award year.  Per your parent(s) IRS Federal Tax Return Transcript they completed a Schedule E ".concat(taxFilingYear).concat(" to report income or loss from rental real estate, royalties, partnerships, S corporations, estates, trusts, and residual interests in Real Estate Mortgage Investment Conduits (REMICs).  In order to determine the amount of financial aid that you are eligible to receive, we must obtain a completed copy of your parents’ Schedule E. See definition of a parent below.&nbsp;<i>Incomplete documents will not be returned. They will be disposed of in a secure manner, per university policy. This will delay processing.</i></p>"));
    var TextTwo = "<p>Submit a photocopy of Schedule E from your parents’ ".concat(taxFilingYear).concat(" Federal Tax Return.  If they reported an income or loss from partnerships and/or S Corporations, attach a copy of Schedule K-1 for each business listed.</p>");

    HeadingYear.value = "PARENT ".concat(taxFilingYear).concat(" FEDERAL TAX RETURN SCHEDULE E (".concat(financialAidYear).concat(")"));
    Year_1.value = financialAidYear;
    Year_2.value = taxFilingYear;
    Year_3.value = taxFilingYear;
...
function checkforDuplicateSubmissions(formCodeVal, financialAidYearVal) {
    ...
    var tableName = "AEM_PARENT_FED_TAX_RETURN_E";
    var financialAidDecisionColumnName = "FINANCIAL_AID_DECISION";
    var formCodeColumnName = "FORM_CODE";
    var faDecision = "Approved";
...
parent_federal_tax_return_schedule_e_parent_federal_tax_return_schedule_e.generated_WorkflowInstanceID_init0 = function (scope) {
	...
	//if(this.value !== null){
		var instance = this.value;
  		var adobeSignDocumentName = 'Parent_Federal_Tax_Return_Schedule_E_Adobe_Sign.pdf';		
...
if(StageIndicator.value === null){
  ...
  EmailSubject.value = "Test - Parent Federal Tax Return Schedule E Form - (" + cwid.value + ")";

  HiddenStudentEmail.value = "shreyas.manjuntha@thoughtfocus.com";

}
  documentNameForAdobeSign.value = "Parent Federal Tax Return Schedule E - " + formCode.value;
guideBridge.submit();
}
        }
	}
