                ...
                $.ajax({
             	...
             	//alert(SSN.value);
              	...
              	maskSSN.value = str.substring(0, 3)+"-"+str.substring(3, 2)+"-"+str.substring(5, 4);*/
                            ...
                            table.id = "tb";
                            var tr = table.insertRow(-1);
                            var headings = ["", "CWID", "Empl_ID", "First_Name", "Last_Name", "EMPL_RCD", "Department ID", "Department Name"];
                            for (var j = 0; j < headings.length; j++) {
                                var th = document.createElement("th");
                                th.innerHTML = headings[j];
                                tr.appendChild(th);
...
/**
 * @function dock_notice_dock_notice.generated_CHRSID_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
dock_notice_dock_notice.generated_CHRSID_init0 = function (scope) {
    with(this) {
        with(scope) {
            ...
            success: function(myresopnse) {
               ...
               // console.log("SSN from EMPLID= "+ maskSSN.value);
                ...
                gifModal.style.display = "none";
                    ...
                    table.id = "tb";
                    var tr = table.insertRow(-1);
                    var headings = ["","CWID", "Empl ID", "First_Name", "Last_Name","EMPL_RCD", "Department ID", "Department Name"];
                    for (var j = 0; j < headings.length; j++) {
                        var th = document.createElement("th");
                        th.innerHTML = headings[j];
                        tr.appendChild(th);
...
/**
 * @function dock_notice_dock_notice.generated_EmplID_init0
 * @this currentComponent
 * @param {scope} scope in which code inside function will be executed.
 */
dock_notice_dock_notice.generated_EmplID_init0 = function (scope) {
    with(this) {
        with(scope) {
            this.enabled=false;
        }
    ...
    //if (StageIndicator.value == "ToHRCoo") {
        if (SupervisorSignature.value === null) {
          /*  var dateString = new Date().toLocaleString("en-US", {
            ...
            SupervisorDate.value = getDateforAdaptiveForm();
...
*/
EmailSubject.value = "Test - Dock Notice Request - "+ LastName.value + ", " + FirstName.value + "("+EmplID.value+")";
EmailSubjectFinal.value = "Test - Dock Notice - Completed ({"+ EmplID.value + "}, Unit: {" + Unit.value + "})";
