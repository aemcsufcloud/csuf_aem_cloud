...
catastrophic_leave_request_catastrophic_leave_request.generated_HRCB_valueCommit0 = function (scope) {
    ...
    //if (StageIndicator.value == "ToHRCoo") {
        if (HRSign.value === null) {
          ...
          if(BargainingUnit.value == "R03"){
            Unit3Date.value = getDateforAdaptiveForm();
            Unit3Date.enabled = false;
          }else{
            StaffUnitDate.value = getDateforAdaptiveForm();
...
if(LastName.value !== null){
	EmailSubject.value = "Test - Catastrophic Leave Request Form - "+ LastName.value;
}else{
 	EmailSubject.value = "Test - Catastrophic Leave Request Form"; 
}
