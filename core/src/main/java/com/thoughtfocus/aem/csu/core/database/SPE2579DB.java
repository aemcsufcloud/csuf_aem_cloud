...
public class SPE2579DB implements WorkflowProcess {

	private static final Logger log = LoggerFactory.getLogger(SPE2579DB.class);
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap processArguments)
		...
		String formName = "Staff Performance Evaluation 2579";
		...
		if (StringUtils.isNotBlank(payloadPath)) {
			try {
				...
				if (null != is) {
					...
					for (int temp = 0; temp < nList.getLength(); temp++) {
						...
						if (nNode.getNodeType() == org.w3c.dom.Node.ELEMENT_NODE) {
							...
							athleticsEmpRating = eElement.getElementsByTagName("AthleticsImpToPos").item(0)
									.getTextContent();
							...
							adminPrintedName = eElement.getElementsByTagName("AdminName").item(0).getTextContent();
				...
				if (conn != null) {
					...
					String resultData = dbUtil.getSPEEvalData(conn, empId,"EMPLID", workflowInstanceId, tableName);
					if (resultData != null && !resultData.equals("")) {
						dbUtil.deleteSPEEvalData(conn, empId,"EMPLID", workflowInstanceId, tableName);
				...
				log.error("Exception in SPE2579DB=" + Arrays.toString(e.getStackTrace()));
			} finally {
				if (null != is)
					try {
						is.close();
						...
						log.error(
								"IOException in SPE2579DB Finally Block=" + Arrays.toString(e.getStackTrace()));
					}
				if (conn != null) {
					try {
						conn.close();
					} catch (SQLException e) {
						log.error("SQLException in SPE2579DB Finally Block=" + Arrays.toString(e.getStackTrace()));
					}
				}
			}
